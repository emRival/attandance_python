...
class Face_Register:
    ...
    def GUI_get_input_name(self):
        ...
        self.input_class_char = self.input_class.get()  # Capture class input
    ...
    def GUI_info(self):
        ...
        tk.Label(self.frame_right_info,
                 font=self.font_step_title,
                 text="Step 2: Input Name and Class").grid(row=7, column=0, columnspan=2, sticky=tk.W, padx=5, pady=20)
        ...
        tk.Button(self.frame_right_info, text='Input', command=self.GUI_get_input_name).grid(row=9, column=2, padx=5)
        ...
        tk.Button(self.frame_right_info,
                  ...
                  command=self.save_current_face).grid(row=11, column=0, columnspan=2, sticky=tk.W)
    ...
    def create_face_folder(self):
        ...
        if self.input_name_char and self.input_class_char:
            # Format: person_x_name_class
            self.current_face_dir = f"{self.path_photos_from_camera}person_{self.existing_faces_cnt}_{self.input_name_char}_{self.input_class_char}"
        else:
